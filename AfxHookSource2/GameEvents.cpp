...
static unsigned int g_GameEventManagerHookAttempts = 0;
...
bool Hook_CGameEventManager(void* addrClientDll) {
    ...
    ++g_GameEventManagerHookAttempts;
...
{
    advancedfx::Message(
        "[mirv_pov_feedback] status GameEvents attempts=%u hooked=%d manager=%p fire=%p clientSide=%p create=%p free=%p\n",
        g_GameEventManagerHookAttempts,
        g_GameEventManagerHooked ? 1 : 0,
        g_pGameEventManager,
        reinterpret_cast<void *>(g_Old_CGameEventManager_FireEvent),
        reinterpret_cast<void *>(g_Old_CGameEventManager_FireEventClientSide),
        reinterpret_cast<void *>(g_CGameEventManager_CreateEvent),
        reinterpret_cast<void *>(g_CGameEventManager_FreeEvent));
}
