...
CViewEffects_ClearFade_t g_CViewEffectsClearFade = nullptr;
    ...
    void * arg9)
{
    ...
    // The native fade hook is installed during client initialization so the
    // POV path can use the same in-game renderer later. Until mirv_pov is
    // enabled, it must be a pure trampoline with no message decoding/logging.
    // Keep the hook a zero-cost trampoline unless mirv_pov is enabled. The
    // silent template sampling above is intentional and does not emit logs.
...
static void RenderSystemDX11_DeathFade_InitializeViewEffects(void * clientDll) {
    ...
    if (nullptr == g_CViewEffectsClearFade) {
        const char * clearPattern =
            "48 89 5C 24 08 57 48 83 EC 20 8B 41 08 48 8B F9 "
            "83 E8 01 48 63 D8";
        size_t address = getAddress((HMODULE)clientDll, clearPattern);
        if (0 != address) g_CViewEffectsClearFade = reinterpret_cast<CViewEffects_ClearFade_t>(address);
    }
}
