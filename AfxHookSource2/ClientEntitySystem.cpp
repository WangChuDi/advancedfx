...
static unsigned int g_MirvPovVoiceLastObserverTarget = SOURCESDK_CS2_INVALID_EHANDLE_INDEX;
...
static void MirvPov_UpdateVoiceTeam() {
    ...
    CEntityInstance * realController = GetRealSplitScreenPlayer(0);
    if(nullptr == realController) return;

    auto pawnHandle = realController->GetPlayerPawnHandle();
    if(!pawnHandle.IsValid()) return;

    CEntityInstance * realPawn = GetEntityFromIndex(pawnHandle.GetEntryIndex());
    if(nullptr == realPawn) return;

    unsigned int * pObsTarget = GetObserverTargetFieldPtrUnchecked(realPawn);
    if(nullptr == pObsTarget) return;

    unsigned int observerTarget = *pObsTarget;
    if(g_MirvPovVoiceLastObserverTarget == observerTarget) return;
    g_MirvPovVoiceLastObserverTarget = observerTarget;

    SOURCESDK::CS2::CBaseHandle targetHandle(observerTarget);
    if(!targetHandle.IsValid()) return;

    CEntityInstance * targetPawn = GetEntityFromIndex(targetHandle.GetEntryIndex());
    if(nullptr == targetPawn) return;
...
void MirvPov_Enable(HMODULE clientDll) {
    ...
    g_MirvPovVoiceLastDemoTick = INT_MIN;
    g_MirvPovVoiceLastObserverTarget = SOURCESDK_CS2_INVALID_EHANDLE_INDEX;
